...
public class WufanGame_Channel extends IChannelProxy {
    ...
    public void roleReport(Activity activity, GameRoleInfo gameRoleInfo, CallbackListener listener) {
        this.u9GameRoleInfo = gameRoleInfo;
        if (gameRoleInfo.getTypeId() == HY_StateType.CREATE_ROLE) {
            PPayCenter.createRole(gameRoleInfo.getRoleName(), gameRoleInfo.getZoneName());
        } else if (gameRoleInfo.getTypeId() == HY_StateType.ENTER_SERVER) {
            PPayCenter.enterGame(gameRoleInfo.getRoleName(), gameRoleInfo.getZoneName());
        }

        Logger.d(TAG, gameRoleInfo.toString());
        Logger.d(TAG, "MethodManager-->setExtData");
    }

    @Override
    public void pay(Activity activity, final PayParams payParams, final PayCallbackListener mPayCallBack) {
...
//        paayArg.APP_SERVER = 1;
        ...
        if (StringUtils.isEmpty(payParams.getAmount() + "")) {
            paayArg.MONEY_AMOUNT = "1.00";
        } else {
            paayArg.MONEY_AMOUNT = payParams.getAmount() / 100 + "";
        }
...
//        paayArg.NOTIFY_URI = "http://sdkapi.papa91.com/index.php/pay_center/test";
        ...
        paayArg.APP_EXT1 = TextUtils.isEmpty(payParams.getAppExtInfo()) ? "ext1 : " + payParams.getGameOrderId() : payParams.getAppExtInfo();
        paayArg.APP_EXT2 = TextUtils.isEmpty(payParams.getAppExtInfo()) ? "ext2 : " + payParams.getGameOrderId() : payParams.getAppExtInfo();
...
//                                mmm = "支付成功";
                        mPayCallBack.onPaySuccess(payParams);
                        break;
